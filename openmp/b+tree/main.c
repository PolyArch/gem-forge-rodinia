...
long transform_to_cuda(node *root, bool verbose) {
  ...
  printf("Tree transformation took %f s\n",
         (double)(timeEnd - timeStart) / 1000000.0);
...
int main(int argc, char **argv) {
  ...
  if (commandFile == NULL) {
    fprintf(stderr, "Command File error");
    exit(1);
  }
  ...
  if (commandBuffer == NULL) {
    fprintf(stderr, "Command Buffer memory error");
    exit(2);
  }
  ...
  if (result != lSize) {
    fprintf(stderr, "Command file reading error");
    exit(3);
  }
  ...
  while (sscanf(commandPointer, "%c", &instruction) != EOF) {
    ...
    case 'k': {
      ...
      // New OpenMP kernel, same algorighm across all versions(OpenMP, CUDA,
      // OpenCL) for comparison purposes
      kernel_cpu(cores_arg,

                 records, knodes, knodes_elem,
    ...
    case 'j': {
      ...
      // New kernel, same algorighm across all versions(OpenMP, CUDA, OpenCL)
