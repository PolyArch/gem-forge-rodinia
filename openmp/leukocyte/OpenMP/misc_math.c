...
MAT *gradient_x(MAT *input) {
  ...
  for (i = 0; i < result->m; i++) {
    for (j = 0; j < result->n; j++) {
      if (j == 0)
        m_set_val(result, i, j,
                  m_get_val(input, i, j + 1) - m_get_val(input, i, j));
      else if (j == input->n - 1)
        m_set_val(result, i, j,
                  m_get_val(input, i, j) - m_get_val(input, i, j - 1));
      else
        m_set_val(result, i, j,
                  (m_get_val(input, i, j + 1) - m_get_val(input, i, j - 1)) /
                      2.0);
    }
  }
...
MAT *gradient_y(MAT *input) {
  ...
  for (i = 0; i < result->n; i++) {
    for (j = 0; j < result->m; j++) {
      if (j == 0)
        m_set_val(result, j, i,
                  m_get_val(input, j + 1, i) - m_get_val(input, j, i));
      else if (j == input->m - 1)
        m_set_val(result, j, i,
                  m_get_val(input, j, i) - m_get_val(input, j - 1, i));
      else
        m_set_val(result, j, i,
                  (m_get_val(input, j + 1, i) - m_get_val(input, j - 1, i)) /
                      2.0);
    }
  }
  return result;
