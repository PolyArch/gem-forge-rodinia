...
MAT *get_frame(avi_t *cell_file, int frame_num, int cropped, int scaled) {
  ...
  if (cropped) {
    ...
    image_chopped = chop_flip_image(image_buf, height, width, TOP, BOTTOM, 0,
                                    width - 1, scaled);
  } else {
    // Just flip the image
    image_chopped = chop_flip_image(image_buf, height, width, 0, height - 1, 0,
                                    width - 1, scaled);
  }
...
MAT *chop_flip_image(unsigned char *image, int height, int width, int top,
                     int bottom, int left, int right, int scaled) {
  ...
  if (scaled) {
    ...
    for (i = 0; i <= (bottom - top); i++)
      for (j = 0; j <= (right - left); j++)
        // m_set_val(result, i, j, (double) image[((height - (i + top)) * width)
        // + (j + left)] * scale);
        m_set_val(
            result, i, j,
            (double)image[((height - 1 - (i + top)) * width) + (j + left)] *
                scale);
  } else {
    for (i = 0; i <= (bottom - top); i++)
      for (j = 0; j <= (right - left); j++)
...
MAT *structuring_element(int radius) {
  ...
  for (i = 0; i < result->m; i++) {
    for (j = 0; j < result->n; j++) {
      if (sqrt((float)((i - radius) * (i - radius) +
                       (j - radius) * (j - radius))) <= radius)
...
void uniformseg(VEC *cellx_row, VEC *celly_row, MAT *x, MAT *y) {
  ...
  for (i = 1; i <= 36; i++) {
    dx[i % 36] =
        v_get_val(cellx_row, i % 36) - v_get_val(cellx_row, (i - 1) % 36);
    dy[i % 36] =
        v_get_val(celly_row, i % 36) - v_get_val(celly_row, (i - 1) % 36);
...
VEC *getsampling(MAT *m, int ns) {
  ...
  for (i = 0; i < N; i++) {
    for (j = 0; j < M; j++) {
      ...
      a = m_get_val(m, 0, aindex[i]) *
          (-1.0 * s * s * s + 3.0 * s * s - 3.0 * s + 1.0);
      b = m_get_val(m, 0, bindex[i]) * (3.0 * s * s * s - 6.0 * s * s + 4.0);
      c = m_get_val(m, 0, cindex[i]) *
          (-3.0 * s * s * s + 3.0 * s * s + 3.0 * s + 1.0);
...
void splineenergyform01(MAT *Cx, MAT *Cy, MAT *Ix, MAT *Iy, int ns,
                        double delta, double dt, int typeofcell) {
  VEC *X, *Y, *Xs, *Ys, *Nx, *Ny, *X1, *Y1, *X2, *Y2, *XY, *XX, *YY, *dCx, *dCy,
      *Ix1, *Ix2, *Iy1, *Iy2;
  ...
  for (i = 0; i < Nx->dim; i++)
    v_set_val(Nx, i,
              v_get_val(Ys, i) / sqrt(v_get_val(Xs, i) * v_get_val(Xs, i) +
                                      v_get_val(Ys, i) * v_get_val(Ys, i)));

  Ny = v_get(Xs->dim);
  for (i = 0; i < Ny->dim; i++)
    v_set_val(Ny, i,
              -1.0 * v_get_val(Xs, i) /
                  sqrt(v_get_val(Xs, i) * v_get_val(Xs, i) +
                       v_get_val(Ys, i) * v_get_val(Ys, i)));
  ...
  for (i = 0; i < Cx->m; i++) {
    for (j = 0; j < ns; j++) {
      ...
      double A1, A2, A3, A4, B1, B2, B3, B4, D, D_3, Tx1, Tx2, Tx3, Tx4, Ty1,
          Ty2, Ty3, Ty4;
      int k;
      ...
      k = i * ns + j;
      D = sqrt(v_get_val(Xs, k) * v_get_val(Xs, k) +
               v_get_val(Ys, k) * v_get_val(Ys, k));
      D_3 = D * D * D;
      ...
      v_set_val(dCx, aindex[i],
                v_get_val(dCx, aindex[i]) + Tx1 * v_get_val(Ix1, k) +
                    Tx2 * v_get_val(Iy1, k) - Tx3 * v_get_val(Ix2, k) -
                    Tx4 * v_get_val(Iy2, k));
      v_set_val(dCy, aindex[i],
                v_get_val(dCy, aindex[i]) + Ty1 * v_get_val(Ix1, k) +
                    Ty2 * v_get_val(Iy1, k) - Ty3 * v_get_val(Ix2, k) -
                    Ty4 * v_get_val(Iy2, k));
      ...
      v_set_val(dCx, bindex[i],
                v_get_val(dCx, bindex[i]) + Tx1 * v_get_val(Ix1, k) +
                    Tx2 * v_get_val(Iy1, k) - Tx3 * v_get_val(Ix2, k) -
                    Tx4 * v_get_val(Iy2, k));
      v_set_val(dCy, bindex[i],
                v_get_val(dCy, bindex[i]) + Ty1 * v_get_val(Ix1, k) +
                    Ty2 * v_get_val(Iy1, k) - Ty3 * v_get_val(Ix2, k) -
                    Ty4 * v_get_val(Iy2, k));
      ...
      v_set_val(dCx, cindex[i],
                v_get_val(dCx, cindex[i]) + Tx1 * v_get_val(Ix1, k) +
                    Tx2 * v_get_val(Iy1, k) - Tx3 * v_get_val(Ix2, k) -
                    Tx4 * v_get_val(Iy2, k));
      v_set_val(dCy, cindex[i],
                v_get_val(dCy, cindex[i]) + Ty1 * v_get_val(Ix1, k) +
                    Ty2 * v_get_val(Iy1, k) - Ty3 * v_get_val(Ix2, k) -
                    Ty4 * v_get_val(Iy2, k));
      ...
      v_set_val(dCx, dindex[i],
                v_get_val(dCx, dindex[i]) + Tx1 * v_get_val(Ix1, k) +
                    Tx2 * v_get_val(Iy1, k) - Tx3 * v_get_val(Ix2, k) -
                    Tx4 * v_get_val(Iy2, k));
      v_set_val(dCy, dindex[i],
                v_get_val(dCy, dindex[i]) + Ty1 * v_get_val(Ix1, k) +
                    Ty2 * v_get_val(Iy1, k) - Ty3 * v_get_val(Ix2, k) -
                    Ty4 * v_get_val(Iy2, k));
    }
  }
