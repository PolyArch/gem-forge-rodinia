...
#pragma omp parallel for num_threads(omp_num_threads) private(i, j)
    ...
    for (cell_num = 0; cell_num < Nc; cell_num++) {
      ...
      for (j = 0; j < Np; j++) {
        ...
        x[cell_num][j][frame_num] =
            xc[cell_num][frame_num] + (ri[j] * cos(t[j]));
        y[cell_num][j][frame_num] =
            yc[cell_num][frame_num] + (ri[j] * sin(t[j]));
      }
  ...
  while ((iter < iterations) && (mean_diff > converge)) {
    ...
    for (i = 0; i < m; i++) {
      for (j = 0; j < n; j++) {
        ...
        // Compute IMGVF += (mu / lambda)(UHe .*U  + DHe .*D  + LHe .*L  + RHe
        // .*R +
        ...
        double vHe = old_val + mu_over_lambda *
  ...
  */
  ...
  while (iter < iterations && snakediff > converge) {
    ...
    for (i = 0; i < n; i++) {
      ...
      double new_val =
          (vf_val + vfx_val * (x_val - xc) + vfy_val * (y_val - yc) - vfmean) /
          L;
    ...
    yc = (yc + (deltay * lambdaedge * vfymean) + (deltay * lambdapath * Ey)) /
