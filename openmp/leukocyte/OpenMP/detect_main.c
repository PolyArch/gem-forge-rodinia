...
int main(int argc, char **argv) {
  ...
  if (argc != 4) {
    fprintf(stderr, "usage: %s <num of frames> <num of threads><input file>",
            argv[0]);
    exit(1);
  }
  ...
  int i, j, *crow, *ccol, pair_counter = 0, x_result_len = 0, Iter = 20, ns = 4,
                          k_count = 0, n;
  MAT *cellx, *celly, *A;
  double *GICOV_spots, *t, *G, *x_result, *y_result, *V, *QAX_CENTERS,
      *QAY_CENTERS;
  ...
  for (n = 0; n < x_result_len; n++) {
    if ((G[n] < -1 * threshold) || G[n] > threshold) {
      ...
      // Make sure that the possible leukocytes are not too close to the edge of
      // the frame
      if ((m_min(x) > b) && (m_min(y) > b) &&
          (m_max(x) < cell_file->width - b) &&
          (m_max(y) < cell_file->height - b)) {
        ...
        for (i = 0; i < Iter; i++) {
          ...
          splineenergyform01(Cx, Cy, grad_x, grad_y, ns, delta, 2.0 * dt,
                             typeofcell);
        }
        ...
        for (i = 0; i < Nx->dim; i++)
          v_set_val(W, i,
                    m_get_val(Ix1, 0, i) * v_get_val(Nx, i) +
                        m_get_val(Iy1, 0, i) * v_get_val(Ny, i));

        V[n] = mean(W) / std_dev(W);

        // get means of X and Y values for all "snaxels" of the spline contour,
        // thus finding the cell centers
  ...
  int num_snaxels = 20;
  ellipsetrack(cell_file, QAX_CENTERS, QAY_CENTERS, k_count, radius,
               num_snaxels, num_frames);
  printf("           Total: %.5f seconds\n",
         ((float)(get_time() - tracking_start_time)) /
             (float)(1000 * 1000 * num_frames));
